...
import hashlib, datetime, rsa, json, copy, requests, MapleTk, os, pathlib, random, threading, psutil, pebble
...
class Interface(Network):
    ...
    def fontInit(self):
        ...
        self.interfaceObjects["Label"]["loginTitle"].config(font=("TimesNewRoman", 38))
        ...
        self.interfaceObjects["Button"]["loginButton"].config(font=("TimesNewRoman", 18))
        self.interfaceObjects["Label"]["welcomeLabel"].config(font=("TimesNewRoman", 18))
        self.interfaceObjects["Label"]["mineBlocksLabel"].config(font=("TimesNewRoman", 11))
        self.interfaceObjects["Label"]["mineBlocksRejectLabel"].config(font=("TimesNewRoman", 11))
        self.interfaceObjects["Label"]["transactionsLabel"].config(font=("TimesNewRoman", 18))
        self.interfaceObjects["Label"]["blockChainLabel"].config(font=("TimesNewRoman", 18))
        self.interfaceObjects["Listbox"]["transactions"].config(font=("TimesNewRoman", 11))
        self.interfaceObjects["Listbox"]["blockChain"].config(font=("TimesNewRoman", 11))
    ...
    def cpusInit(self):
        ...
        sizeCalc, startCalc= lambda x : 15 if x <= 8 else 13 if x <= 16 else 7, lambda x: 0.04 if x <= 8 else 0.011
    ...
    def renderTransactionsBox(self):
        ...
        listboxSize = self.interfaceObjects["Listbox"]["transactions"].size()
        for i in range(listboxSize):
            self.interfaceObjects["Listbox"]["transactions"].delete(i)
    ...
    def renderBlockChainBox(self):
        ...
        #Deleting all Enteries in the list box (Resetting)
        listboxSize = self.interfaceObjects["Listbox"]["blockChain"].size()
        for i in range(listboxSize):
            self.interfaceObjects["Listbox"]["blockChain"].delete(i)
    ...
    def showMiningScreen(self):
        ...
        self.interfaceObjects["Master"].geometry("1520x855")

        #Show the Mining Screen Widgets
        widgetsToShow = ("blockChain", "blockChainLabel", "transactions", "transactionsLabel", "navigationFrame", "welcomeLabel", "mineBlocksLabel", "mineBlocksRejectLabel", "CPUFrame", "refreshCPUs", "refreshData")
...
class cpu(Network):
    ...
    def callBack(self, fn):
        ...
        if not fn.cancelled():
            ...
            #Update interface according to staus
            if self._status == "OK":
                self.blocksMined +=1
